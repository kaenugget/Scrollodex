  ...
  handler: async (ctx, args) => {
    ...
    if (filterType) {
      entries = entries.filter(entry => 
        entry.types.includes(filterType)
      );
    }
...
export const computeEntry = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    contact.tags.forEach(tag => {
      ...
      if (lowerTag.includes("art") || lowerTag.includes("creative")) types.push("ART");
      if (lowerTag.includes("tech") || lowerTag.includes("engineer")) types.push("ELEC");
      if (lowerTag.includes("music") || lowerTag.includes("sound")) types.push("PSY");
      if (lowerTag.includes("business") || lowerTag.includes("sales")) types.push("STEEL");
      if (lowerTag.includes("design") || lowerTag.includes("visual")) types.push("GRASS");
      if (lowerTag.includes("food") || lowerTag.includes("cooking")) types.push("FIRE");
      if (lowerTag.includes("water") || lowerTag.includes("ocean")) types.push("WATER");
    });
    
    // Default to NORM if no types found
    if (types.length === 0) types.push("NORM");
...
export const updateXp = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    let newLevel = 1;
    if (newXp >= 25) newLevel = 3;
    else if (newXp >= 10) newLevel = 2;

...
async function generatePetVideosBackground(ctx: any, contactId: any, petData: any) {
    ...
    for (let attempt = 1; attempt <= 2; attempt++) {
      try {
        ...
        const timeoutPromise = new Promise((_, reject) => 
          setTimeout(() => reject(new Error('Video generation timeout after 5 minutes')), 300000) // 5 minutes timeout
        );

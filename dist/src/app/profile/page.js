...
import { User, Bell, Shield, Palette, Database, Download, Trash2, Mail, QrCode, Copy, Check } from 'lucide-react';
export default function ProfilePage() {
    ...
    return (<main className="scrollodex-bg">
      <AnimatedBackground />
      <AppHeader currentPage="profile" onNavigate={(page) => {
            if (page === 'contacts' || page === 'dex' || page === 'home') {
                window.location.href = '/';
            }
        }} user={currentUser} onSignOut={handleSignOut}/>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 sm:gap-8">
          ...
          <div className="lg:col-span-1">
            <div className="scrollodex-card">
              <nav className="space-y-2">
                {tabs.map(({ key, label, icon: Icon }) => (<button key={key} onClick={() => setActiveTab(key)} className={`w-full flex items-center gap-3 px-4 py-3 text-left rounded-lg transition-colors ${activeTab === key
          ...
          <div className="lg:col-span-3">
            {activeTab === 'profile' && (<div className="scrollodex-card-large">
                ...
                <div className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      ...
                      <input type="text" defaultValue={currentUser.displayName || ''} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="Enter your display name"/>
                    </div>
                    <div>
                      <label className="block text-sm font-medium scrollodex-text-dark mb-2">Email</label>
                      <input type="email" value={currentUser?.email || 'No email provided'} disabled className="w-full px-3 py-2 border border-gray-300 rounded-lg bg-gray-50 scrollodex-text-light-gray"/>
                    </div>
                  </div>

                  <div>
                    <label className="block text-sm font-medium scrollodex-text-dark mb-2">Bio</label>
                    <textarea rows={4} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="Tell us about yourself..."/>
                  </div>
                  ...
                  <div className="border-t pt-6">
                    ...
                    <div className="flex gap-3">
                      <Button onClick={generateProfileShareLink} className="bg-emerald-600 hover:bg-emerald-700 text-white">
                    ...
                    {/* QR Code Modal */}
                    {showQR && shareUrl && (<div className="scrollodex-card mt-6 text-center">
                        ...
                        <div className="flex gap-2 justify-center">
                          <Button variant="outline" size="sm" onClick={copyToClipboard} className="border-gray-300 text-gray-700 hover:bg-gray-50">
                            {copied ? <Check className="w-4 h-4 mr-1"/> : <Copy className="w-4 h-4 mr-1"/>}
                            {copied ? "Copied!" : "Copy Link"}
                          </Button>
                          <Button variant="outline" size="sm" onClick={() => setShowQR(false)} className="border-gray-300 text-gray-700 hover:bg-gray-50">
                            Close
                          </Button>
                        </div>
                      </div>)}
            ...
            {activeTab === 'notifications' && (<div className="scrollodex-card-large">
            ...
            {activeTab === 'privacy' && (<div className="scrollodex-card-large">
            ...
            {activeTab === 'appearance' && (<div className="scrollodex-card-large">
                ...
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <div className="flex gap-3">
                      {['blue', 'green', 'purple', 'red', 'orange'].map(color => (<div key={color} className={`w-8 h-8 rounded-full bg-${color}-500 cursor-pointer border-2 ${color === 'blue' ? 'border-gray-900' : 'border-transparent'}`}/>))}
                    </div>
                  </div>
                </div>
              </div>)}

            {activeTab === 'data' && (<div className="scrollodex-card-large">

...
export function PeerPageView({ peerPageId }) {
    ...
    // Get peer page data
    const peerPage = useQuery(api.social.getPeerPage, { peerPageId: peerPageId });
    ...
    const hasAccess = isUserAuthenticated && currentUser && (currentUser._id === peerPage.aUserId ||
    ...
    return (<main className="min-h-screen bg-gray-50">
      <AppHeader currentPage="home" onNavigate={(page) => {
            if (page === 'dex') {
                window.location.href = '/';
            }
      ...
      <div className="bg-white border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex gap-1 bg-gray-100 p-1 rounded-lg">
            {tabs.map((tab) => {
            const Icon = tab.icon;
            return (<Button key={tab.id} variant={activeTab === tab.id ? "default" : "ghost"} onClick={() => setActiveTab(tab.id)} className={`flex items-center gap-2 px-6 py-3 font-medium text-sm transition-colors rounded-md ${activeTab === tab.id
      ...
      {/* Content */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {activeTab === "moments" && (<MomentsFeed peerPageId={peerPageId}/>)}
        {activeTab === "deck" && (<DeckGrid peerPageId={peerPageId}/>)}
            ...
            <Card className="bg-white border-gray-200 p-6">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input type="text" value={peerPage.title} readOnly className="w-full px-3 py-2 bg-gray-50 border border-gray-300 rounded-md text-gray-900"/>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    Visibility
...
//# sourceMappingURL=PeerPageView.js.map
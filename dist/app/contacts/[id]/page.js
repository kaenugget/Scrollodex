...
export default function ContactDetailPage() {
    ...
    const { contacts, isLoading: contactsLoading } = useContacts(currentUser?._id, !!currentUser);
    const { dexEntries, isLoading: dexLoading } = useDexEntries(currentUser?._id, !!currentUser);
    const handleSignOut = () => {
        if (isSignedIn) {
    ...
    return (<main className="scrollodex-bg">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-8">
        <ContactDetailView contactId={contactId} userId={currentUser._id} activeTab={activeTab} onTabChange={setActiveTab}/>
      </div>
    </main>);
}

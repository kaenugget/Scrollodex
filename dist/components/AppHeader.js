...
const NavButton = ({ label, isActive, onClick }) => (<button onClick={onClick} className={`font-medium text-sm px-4 py-2 transition-colors duration-200 ${isActive
        ? 'text-blue-600 border-b-2 border-blue-600'
        : 'text-gray-600 hover:text-gray-900'}`}>
        {label}
    </button>);
export const AppHeader = ({ currentPage, onNavigate, user, onSignOut }) => {
    const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false);
    return (<header className="bg-white border-b border-gray-200 sticky top-0 z-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          {/* Desktop User Menu */}
          {user && onSignOut && (<div className="hidden md:block">
              <UserDropdown user={user} onSignOut={onSignOut}/>
            </div>)}

          {/* Mobile Menu Button */}
          <div className="md:hidden flex items-center space-x-4">
            {user && onSignOut && (<UserDropdown user={user} onSignOut={onSignOut}/>)}
        ...
        {isMobileMenuOpen && (<div className="md:hidden border-t border-gray-200 py-4">
            <nav className="flex flex-col space-y-2">
              <NavButton label="Dex" isActive={currentPage === 'dex'} onClick={() => {
                onNavigate('dex');

...
export function MomentsFeed({ peerPageId }) {
    ...
    return (<div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <Button onClick={() => setShowAddMoment(true)} className="bg-emerald-600 hover:bg-emerald-700 text-white">
      ...
      {/* Moments Grid */}
      {moments.length === 0 ? (<Card className="bg-gray-800 border-gray-700 p-12 text-center">
      ...
      {/* Add Moment Sheet */}
      {showAddMoment && (<AddMomentSheet peerPageId={peerPageId} onClose={() => setShowAddMoment(false)}/>)}
...
function MomentCard({ moment, onPhotoClick }) {
    ...
    const isLiked = useQuery(api.social.isMomentLiked, currentUser ? { momentId: moment._id, userId: currentUser._id } : "skip");
    ...
    if (!photoUrl) {
        return (<Card className="bg-gray-800 border-gray-700 p-4">
    ...
    return (<Card className="bg-gray-800 border-gray-700 overflow-hidden">
      {/* Photo */}
      <div className="aspect-square bg-gray-700 cursor-pointer group relative" onClick={() => onPhotoClick(photoUrl)}>
        ...
        {moment.visibility && moment.visibility !== "private" && (<div className="absolute top-2 right-2">
      ...
      <div className="p-4 space-y-3">
        ...
        {/* Caption */}
        {moment.caption && (<p className="text-white text-sm leading-relaxed">{moment.caption}</p>)}

        {/* Tags */}
        {moment.tags && moment.tags.length > 0 && (<div className="flex flex-wrap gap-1">
            {moment.tags.slice(0, 3).map((tag) => (<Badge key={tag} variant="secondary" className="text-xs bg-emerald-600 text-white">
                <Tag className="w-3 h-3 mr-1"/>
                {tag}
              </Badge>))}
            {moment.tags.length > 3 && (<Badge variant="secondary" className="text-xs bg-gray-600 text-gray-300">
        ...
        <div className="flex items-center gap-3 text-sm text-gray-400">
          {moment.mood && (<div className="flex items-center gap-1">
              <Smile className="w-4 h-4"/>
              <span>{moodEmojis[moment.mood] || "😊"} {moment.mood}</span>
            </div>)}
          {moment.weather && (<div className="flex items-center gap-1">
              <Cloud className="w-4 h-4"/>
              <span>{weatherEmojis[moment.weather] || "☀️"} {moment.weather}</span>
            </div>)}
          {moment.activity && (<div className="flex items-center gap-1">
        ...
        {/* Location */}
        {moment.placeName && (<div className="flex items-center gap-2 text-gray-400 text-sm">
        ...
        <div className="flex items-center gap-4 pt-2 border-t border-gray-700">
          <button onClick={handleLike} className={`flex items-center gap-1 text-sm transition-colors ${isLiked ? "text-red-400" : "text-gray-400 hover:text-red-400"}`}>
...
//# sourceMappingURL=MomentsFeed.js.map
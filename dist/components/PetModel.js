...
export function PetModel({ contactId, userId, relationshipStats, petData }) {
    const [isGenerating, setIsGenerating] = useState(false);
    const [petModel, setPetModel] = useState(petData);
    const [showEvolve, setShowEvolve] = useState(false);
    const [connectionStatus, setConnectionStatus] = useState('unknown');
    const [debugStyle, setDebugStyle] = useState(null);
    ...
    const getCustomizationOptions = useQuery(api.pets.getCustomizationOptions, petData?.petType ? { petType: petData.petType } : "skip");
    const debugPetData = useQuery(api.pets.debugPetData, contactId ? { contactId } : "skip");
    // Video generation hook
    const { videoStatus, isPolling, handleStartVideoGeneration, getTimeElapsed, getEstimatedTimeRemaining, isGenerating: isVideoGenerating, isCompleted: isVideoCompleted, isFailed: isVideoFailed, isPending: isVideoPending, hasVideos, } = useVideoGeneration(contactId, userId);
    ...
    const getPetState = () => {
        if (!petModel?.happiness)
            return 'neutral';
        if (petModel.happiness >= 80)
            return 'excited';
        if (petModel.happiness >= 60)
            return 'happy';
        if (petModel.happiness >= 40)
            return 'neutral';
        return 'sad';
    };
    ...
    const getCurrentPetVideo = () => {
        ...
        return petModel?.[`${state}VideoUrl`] || petModel?.neutralVideoUrl;
    };
    ...
    const getCurrentPetImage = () => {
        const state = getPetState();
        const imageUrl = petModel?.[`${state}ImageUrl`] || petModel?.neutralImageUrl;
    ...
    const handleHatchPet = async () => {
        ...
        try {
            ...
            const result = await Promise.race([
                ...
                new Promise((_, reject) => setTimeout(() => reject(new Error('Pet generation timeout after 3 minutes. The process may still be running in the background.')), 180000))
            ]);
    ...
    const handleEvolve = async (customizationType, newValue) => {
        ...
        try {
            ...
            const result = await Promise.race([
                ...
                new Promise((_, reject) => setTimeout(() => reject(new Error('Evolution timeout after 2 minutes. The process may still be running in the background.')), 120000))
            ]);
    ...
    return (<div className="bg-white rounded-xl shadow-sm p-6">
      ...
      <div className="relative bg-gradient-to-br from-purple-50 to-pink-50 rounded-lg p-8 mb-4 min-h-[200px] flex items-center justify-center">
        {isLoadingPetData ? (<div className="text-center">
            <LoadingSpinner size="xl" className="mx-auto mb-4"/>
            <div className="text-sm text-gray-600">Loading pet data...</div>
          </div>) : isGenerating ? (<div className="text-center">
            <LoadingSpinner size="xl" className="mx-auto mb-4"/>
          ...
          </div>) : hasExistingPet ? (<div className="text-center">
            {/* Pet Video/Image */}
            <div className="w-32 h-32 mb-4 mx-auto rounded-full overflow-hidden bg-white shadow-lg">
              {(() => {
                ...
                return videoUrl && videoUrl.trim() !== "" ? (<video src={videoUrl} autoPlay loop muted playsInline className="w-full h-full object-cover" onError={(e) => {
                    ...
                    }} onLoadStart={() => console.log('🐣 PetModel: Video loading started:', videoUrl)} onCanPlay={() => console.log('🐣 PetModel: Video can play:', videoUrl)}/>) : imageUrl && imageUrl.trim() !== "" ? (<img src={imageUrl} alt={`${petModel.petName} the ${petModel.petType}`} className="w-full h-full object-cover" onError={(e) => {
                      ...
                      `;
                    }} onLoad={() => console.log('🐣 PetModel: Image loaded successfully:', imageUrl)}/>) : (<div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-purple-400 to-pink-400">
            ...
            <div className="text-xs mt-1">
              {isVideoCompleted && hasVideos && (<div className="text-purple-600 flex items-center justify-center gap-1">
                  <Video className="w-3 h-3"/>
                  ✨ Animated Pet
                </div>)}
              {isVideoGenerating && (<div className="text-blue-600 flex items-center justify-center gap-1">
                  <Clock className="w-3 h-3 animate-spin"/>
                  Generating videos... ({getTimeElapsed()}s)
                </div>)}
              {isVideoFailed && (<div className="text-red-600 flex items-center justify-center gap-1">
                  <XCircle className="w-3 h-3"/>
                  Video generation failed
                </div>)}
              {isVideoPending && !hasVideos && (<div className="text-gray-500 flex items-center justify-center gap-1">
                  <Video className="w-3 h-3"/>
                  Videos pending
                </div>)}
            </div>
            ...
            <button onClick={handleHatchPet} disabled={isGenerating || hasExistingPet} className={`px-4 py-2 text-sm font-medium rounded-lg transition-colors ${hasExistingPet
      ...
      {/* Pet Stats */}
      {hasExistingPet && (<div className="space-y-3 mb-4">
          ...
          <div className="w-full bg-gray-200 rounded-full h-2">
            <div className="bg-gradient-to-r from-red-400 to-pink-500 h-2 rounded-full transition-all duration-300" style={{ width: `${petHappiness}%` }}></div>
          </div>

          {/* Action Buttons */}
          <div className="flex justify-center mt-3">
            <div className="flex gap-2 flex-wrap justify-center">
              <button onClick={() => setShowEvolve(true)} disabled={isGenerating} className="px-6 py-2 bg-purple-100 text-purple-700 text-sm font-medium rounded-lg hover:bg-purple-200 transition-colors flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed">
                <Star className="w-4 h-4"/>
                Evolve Your Pet
              </button>
              <button onClick={handleHatchPet} disabled={isGenerating} className="px-4 py-2 bg-blue-100 text-blue-700 text-sm font-medium rounded-lg hover:bg-blue-200 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                {isGenerating ? 'Regenerating...' : 'Regenerate Pet'}
              </button>
              <button onClick={handleGenerateVideos} disabled={isGenerating || isVideoGenerating} className={`px-4 py-2 text-sm font-medium rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed ${isVideoGenerating
                ? 'bg-blue-100 text-blue-700'
                : isVideoCompleted
                    ? 'bg-green-100 text-green-700 hover:bg-green-200'
                    : 'bg-green-100 text-green-700 hover:bg-green-200'}`}>
                {isVideoGenerating ? (<div className="flex items-center gap-2">
                    <Clock className="w-4 h-4 animate-spin"/>
                    Generating Videos...
                  </div>) : isVideoCompleted ? (<div className="flex items-center gap-2">
                    <CheckCircle className="w-4 h-4"/>
                    Videos Ready
                  </div>) : isVideoFailed ? (<div className="flex items-center gap-2">
      ...
      {/* Evolution Panel */}
      {showEvolve && hasExistingPet && (<div className="mt-4 p-4 bg-purple-50 rounded-lg border border-purple-200">
          <div className="flex items-center justify-between mb-3">
            <h4 className="text-sm font-semibold text-purple-900">Evolve Your Pet</h4>
            <button onClick={() => setShowEvolve(false)} className="text-purple-600 hover:text-purple-800 text-sm">
              ✕
            </button>
          </div>
          ...
          {getCustomizationOptions && (<div className="space-y-3">
              {/* Color Evolution */}
              <div>
                <label className="text-xs text-purple-700 font-medium">Change Color</label>
                <div className="flex gap-2 mt-1 flex-wrap">
                  {getCustomizationOptions.colors.map((color) => (<button key={color} onClick={() => handleEvolve('color', color)} disabled={isGenerating} className={`w-8 h-8 rounded-full border-2 ${petModel?.color === color ? 'border-purple-800' : 'border-gray-300'} disabled:opacity-50 disabled:cursor-not-allowed`} style={{ backgroundColor: color }} title={`Change to ${color}`}/>))}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex gap-2 mt-1 flex-wrap">
                  {getCustomizationOptions.patterns.map((pattern) => (<button key={pattern} onClick={() => handleEvolve('pattern', pattern)} disabled={isGenerating} className={`px-3 py-1 text-xs rounded border ${petModel?.pattern === pattern
                        ? 'bg-purple-600 text-white border-purple-600'
                        : 'bg-white text-gray-700 border-gray-300'} disabled:opacity-50 disabled:cursor-not-allowed`}>
                      {pattern}
                    </button>))}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex gap-2 mt-1 flex-wrap">
                  {getCustomizationOptions.accessories.map((accessory) => (<button key={accessory} onClick={() => handleEvolve('accessory', accessory)} disabled={isGenerating} className={`px-3 py-1 text-xs rounded border ${petModel?.accessory === accessory
                        ? 'bg-purple-600 text-white border-purple-600'
                        : 'bg-white text-gray-700 border-gray-300'} disabled:opacity-50 disabled:cursor-not-allowed`}>
                      {accessory}
                    </button>))}
              ...
              <div>
                ...
                <div className="mt-1">
                  <input type="text" defaultValue={petModel?.petName || ''} placeholder="Enter new name" className="w-full px-2 py-1 text-sm border border-gray-300 rounded" onKeyDown={(e) => {
          ...
          {isGenerating && (<div className="mt-3 p-2 bg-purple-100 rounded text-xs text-purple-800 text-center">
              🔄 Evolving your pet...
            </div>)}
        </div>)}
      ...
      <div className="mt-4 p-3 bg-yellow-50 rounded-lg border border-yellow-200">
        <div className="text-xs text-yellow-800">
          ...
          <div className="mt-2 space-y-1 text-xs">
            ...
            <div>Connection Status: {connectionStatus === 'connected' ? '✅ Connected' :
            ...
            <div className="mt-2">
              ...
              <div className="text-xs ml-2">
                ...
                {videoStatus.error && (<div className="text-red-600">Error: {videoStatus.error}</div>)}
              </div>
            </div>
          </div>
          <div className="mt-2">
            ...
            <div className="flex gap-1 mb-2 flex-wrap">
              {['happy', 'neutral', 'sad', 'excited'].map((style) => (<button key={style} onClick={() => setDebugStyle(style)} className={`px-2 py-1 text-xs rounded transition-colors ${debugStyle === style
                ? 'bg-yellow-200 text-yellow-900 font-semibold'
                : 'bg-yellow-100 text-yellow-700 hover:bg-yellow-200'}`}>
                  {style.charAt(0).toUpperCase() + style.slice(1)}
                </button>))}
              <button onClick={() => setDebugStyle(null)} className={`px-2 py-1 text-xs rounded transition-colors ${debugStyle === null
            ...
            <div className="text-xs text-yellow-700 mb-2">
              ...
              {debugStyle && (<span className="ml-2 text-yellow-600">
                  (Testing {debugStyle} state)
                </span>)}
            </div>
          </div>
          <div className="mt-2 flex gap-2">
            <button onClick={testConnection} className="px-2 py-1 bg-blue-100 text-blue-700 text-xs rounded hover:bg-blue-200">
              Test Connection
            </button>
            <button onClick={() => console.log('🐣 PetModel: Full state dump:', {
            petData,
            petModel,
            hasExistingPet,
            isLoadingPetData,
            ...
            <button onClick={() => console.log('🐛 Debug Pet Data:', debugPetData)} className="px-2 py-1 bg-purple-100 text-purple-700 text-xs rounded hover:bg-purple-200">
              Debug Pet Data
            </button>
          </div>
        </div>
